...
@Configurable
public class RedCloseHori extends CommandBase {
    public static boolean finished = false;
    public static int duration = 1000;
    public static Pose hori = new Pose(72, 133, Math.toRadians(95));
    public static ElapsedTime timer;

    private final Drivetrain drivetrain;
    private final Follower follower;

    public RedCloseHori(final Drivetrain drivetrain) {
...
//        new PathCommand(this.drivetrain, traj).schedule();
    ...
    public boolean isFinished() {
        if (finished && timer.milliseconds() > duration)
            ...
            if ((Math.abs(follower.getPose().getY() - hori.getY()) < 3) && (Math.abs(follower.getPose().getX() - hori.getX()) < 3) && (Math.abs(Math.toDegrees(
